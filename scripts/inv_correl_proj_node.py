...
class InverseTriangulationNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('threshold', 0.9)
        ...
        self.callback_group_black_proj_client = MutuallyExclusiveCallbackGroup()
        self.callback_group_project_client = MutuallyExclusiveCallbackGroup()
        ...
        self.pattern_client = self.create_client(Trigger, 'next_image', callback_group=self.callback_group_project_client)
        self.black_client = self.create_client(Trigger, 'black_image', callback_group=self.callback_group_project_client)
    ...
    def get_images_srv(self, request, response):
        ...
        for _ in range(self.num_images):
            future = self.gpio_client.call_async(Trigger.Request())
            rclpy.spin_until_future_complete(self, future)
            if future.result() is not None:
                self.pattern_client.call_async(Trigger.Request())
                time.sleep(0.01)
            else:
        ...
        time.sleep(0.1)
        self.black_client.call_async(Trigger.Request())
        response.success = True
    ...
    def spatial_3d_correl_process(self):
        ...
        GRID_STEPS_1 = {'xy': 2.0, 'z': 4.0} # first steps of 3d patch
